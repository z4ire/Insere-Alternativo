...
def criar_nova_versao(wb, versao_atual, versao_intermediaria, nova_versao, n_nova_versao, BOM, codigo_principal, responsavel):
    ...
    if versao_lista_celula is not None:
        ...
        messagebox.showerror(
            message=f"Não foi encontrado a palavra \"{buscar['Versão da lista']}\" na coluna \"A\" da versão \"{versao_atual}\" da BOM {BOM}", title="ERRO")
        raise ValueError(
            f"Não foi encontrado a palavra \"{buscar['Versão da lista']}\" na coluna \"A\" da versão \"{versao_atual}\" da BOM {BOM}")
